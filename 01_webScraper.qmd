...
## Preview
* Web scraping data via BeautifulSoup
...
* The initial ETL
* Reviewing Data Structure & Distribution

## Let's Get Started!
This analysis was performed using Python in a Jupyter Notebook environment. 
...
### Inspecting HTML
Use the latest URL (for the current year) to inspect the HTML table structure: headings and rows. You’ll notice there are 26 headings. The heading text is clean, so we can store it to create a DataFrame later.
Within the table rows, many cells contain blank spaces and the text 'N/A', which we will clean that up shortly.
...
After reviewing the table structure, we can iterate through all the URLs to extract the full dataset. All data is stored in a dataframe and exported as a CSV file for further analysis.
...
Box plots allow us to view the distribution of rates for each maturity period.
...
([US Treasury Bond Inverted Yield Curve](02_invertedYieldCurve.qmd))